...
def build_trainer(args, device_id, model, optims,loss):
    ...
    if (model):
        ...
        logger.info('* number of trainable parameters: %d' % _tally_trainable_parameters(model))
...
class Trainer(object):
    ...
    def test(self, test_iter, step, cal_lead=False, cal_oracle=False):
        ...
        if(step!=-1 and self.args.report_rouge):
            ...
            rouges = scores.compute_scores(can_path, gold_path, newline_token='<q>')
    ...
    def _count_checkpoints(self):
        """ Counts the number of saved checkpoints and removes the oldest ones if necessary. """
        checkpoints = sorted(glob.glob(os.path.join(self.args.model_path, 'model_step_*.pt')), key=os.path.getmtime)
        if len(checkpoints) > self.num_checkpoints_to_keep > 0:
            num_checkpoints_to_remove = len(checkpoints) - self.num_checkpoints_to_keep
            checkpoints_to_remove = checkpoints[:num_checkpoints_to_remove]
