...
from sentsumm.loss import sentsumm_loss
    ...
    if (args.test_all):
        ...
        for i, cp in enumerate(cp_files):
            ...
            if (args.test_start_from != -1 and step < args.test_start_from):
                rouge_scores.append((0, cp))
                continue
            rouge1, rouge2, rougeL = validate_rouge(args, device_id, cp, step)
            rouge_scores.append(((rouge1, rouge2, rougeL), cp))
        (rouge1, rouge2, rougeL), cp = max(rouge_scores, key=lambda x: sum(x[0]))
        logger.info('Selected checkpoint %s with ROUGE scores: (%.2f, %.2f, %.2f)' % (cp, rouge1, rouge2, rougeL))
        step = int(cp.split('.')[-2].split('_')[-1])
...
def validate(args, device_id, pt, step):
    ...
    tokenizer = BertTokenizer.from_pretrained(args.pretrained_dir, do_lower_case=True)
    symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}

    if args.model == 'presumm':
        model = AbsSummarizer(args, device, checkpoint)
    elif args.model == 'sentsumm':
        model = SentSumm(args, device, symbols, checkpoint)
    else:
...
def validate_rouge(args, device_id, pt, step, num_batches=500):
    """ Computes rouge scores on the first `num_batches` batches of the validation set. """
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    if (pt != ''):
        test_from = pt
    else:
    ...
    return predictor.translate(valid_iter, step, max_batches=num_batches)
...
def train_abs_single(args, device_id):
    ...
    if args.train_from != '':
        ...
        # do not overwrite optimizer params
        logger.info('Loading checkpoint from %s' % args.train_from)
    ...
    if args.model == 'presumm':
        ...
        model = SentSumm(args, device, symbols, checkpoint, bert_from_extractive)
    else:
        raise ValueError('Unknown model: %s' % args.model)
    if args.do_finetune:
        # do not overwrite optimizer params
    ...
    if (args.sep_optim):
        ...
        # only add optimizer if it has parameters to train
        optim = []
        if hasattr(optim_bert, 'optimizer'):
            optim.append(optim_bert)
        if hasattr(optim_dec, 'optimizer'):
            optim.append(optim_dec)
    else:
        optim = [model_builder.build_optim(args, model, checkpoint)]

    logger.info(model)

    if args.sentplan_loss_weight > 0:
        train_loss = sentsumm_loss(model.generator, symbols, model.vocab_size, device, train=True,
                                   label_smoothing=args.label_smoothing, vocab=tokenizer,
                                   sentplan_loss_weight=args.sentplan_loss_weight)
    else:
