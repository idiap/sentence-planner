...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, n_words=0, n_correct=0):
        ...
        self.start_time = time.time()
        self.xent_loss = 0
        self.sentplan_loss = 0
        self.sentplan_loss_weighted = 0
        self.total_loss = 0
    ...
    def update(self, stat, update_n_src_words=False):
        ...
        self.xent_loss += stat.xent_loss
        self.sentplan_loss += stat.sentplan_loss
        self.sentplan_loss_weighted += stat.sentplan_loss_weighted
    ...
    def xent_loss_share(self):
        """ Cross-entropy loss share of total loss. """
        return self.xent_loss / self.total_loss if self.total_loss > 0 else 0

    def sentplan_loss_share(self):
        """ Sentence planner loss share of total loss. """
        return self.sentplan_loss_weighted / self.total_loss if self.total_loss > 0 else 0
    ...
    def log_tensorboard(self, prefix, writer, learning_rate, step):
        ...
        if self.xent_loss:
            writer.add_scalar(prefix + "/xent_loss", self.xent_loss, step)
            writer.add_scalar(prefix + "/xent_loss_share", self.xent_loss_share(), step)
        if self.sentplan_loss:
            writer.add_scalar(prefix + "/sentplan_loss", self.sentplan_loss, step)
            writer.add_scalar(prefix + "/sentplan_loss_weighted", self.sentplan_loss_weighted, step)
            writer.add_scalar(prefix + "/sentplan_loss_share", self.sentplan_loss_share(), step)
        if self.total_loss:
